...
import 'package:your_app/components/ui/kbd.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: const [
        GalaxyKbd(child: Text('Ctrl')),
        SizedBox(width: 8),
        Text('+'),
        SizedBox(width: 8),
        GalaxyKbd(child: Text('K')),
      ],
    );
  }
}
...
::: code-group
```bash [React]
npx @galaxy-stack/design-cli add kbd
```
...
|------|------|---------|-------------|------------|
| `size` | `'sm' \| 'default' \| 'lg'` | `'default'` | Size variant | All |
| `className` / `class` | `string` | `""` | CSS class names | React (className), Vue/Angular (class) |
| `children` | `ReactNode` | - | Keyboard key label or shortcut content | All |
